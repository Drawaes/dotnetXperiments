...
namespace Mono.Cecil {
	...
	sealed class MetadataReader : ByteBuffer {
		...
		public bool HasInterfaces (TypeDefinition type)
		{
            ...
            return metadata.TryGetInterfaceMapping(type, mapping: out Collection<Row<uint, MetadataToken>> mapping);
        }
		...
		public bool HasGenericConstraints (GenericParameter generic_parameter)
		{
            ...
            if (!metadata.TryGetGenericConstraintMapping(generic_parameter, out Collection<MetadataToken> mapping))
                return false;
		...
		public IEnumerable<MemberReference> GetMemberReferences ()
		{
            ...
            {
                DeclaringType = new TypeReference(string.Empty, string.Empty, module, type_system.CoreLibrary)
            };
			...
			for (int i = 0; i < rows.Length; i++) {
				if (rows [i].Col1 == StateMachineScopeDebugInformation.KindIdentifier) {
                    ...
                    var async_body = new AsyncMethodBodyDebugInformation(catch_offset)
                    {
                        yields = yields,
                        resumes = resumes,
                        move_next = GetMethodDefinition(move_next_rid)
                    };
	...
	sealed class SignatureReader : ByteBuffer {
		...
		public void ReadMethodSignature (IMethodSignature method)
		{
            ...
            if (method is MethodReference method_ref)
